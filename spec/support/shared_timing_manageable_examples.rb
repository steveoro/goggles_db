...
shared_examples_for 'TimingManageable' do
  ...
  context 'by including this concern, the sibling:' do
    subject { fixture_row }
    it_behaves_like('responding to a list of methods', %i[hundreds seconds minutes to_timing from_timing])
  end
  ...
  describe '#from_timing' do
    let(:new_hundreds) { ((rand * 100) % 99).to_i }
    ...
    let(:new_timing) { Timing.new(new_hundreds, new_seconds, new_minutes, new_hours, new_days) }
    ...
    it 'sets the internal time members with the data from the fixture' do
      expect(subject.hundreds).to eq(new_hundreds)
