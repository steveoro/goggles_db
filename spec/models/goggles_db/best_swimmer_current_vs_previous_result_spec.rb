# frozen_string_literal: true

require 'rails_helper'
require 'support/shared_abstract_best_result_examples'

module GogglesDb # rubocop:disable Metrics/ModuleLength
  RSpec.describe BestSwimmerCurrentVsPreviousResult do
    context 'shared behaviors' do
      it_behaves_like('an AbstractBestResult descendant', described_class)
      it_behaves_like('AbstractBestResult filtering scopes', described_class)
      it_behaves_like('AbstractBestResult sorting scopes', described_class)
    end

    describe 'old result columns' do
      let(:event_type) { GogglesDb::EventType.find(11) }
      let(:gender_type) { GogglesDb::GenderType.male }
      let(:pool_type) { GogglesDb::PoolType.find(1) }

      def create_current_meeting_session
        current_begin_date = Date.new(2999, 1, 1)
        current_end_date = Date.new(2999, 12, 31)
        current_season = create(
          :season,
          season_type_id: 1,
          begin_date: current_begin_date,
          end_date: current_end_date,
          header_year: "#{current_begin_date.year}/#{current_end_date.year}"
        )
        current_meeting = create(
          :meeting,
          season: current_season,
          header_date: current_begin_date + 90.days
        )
        create(:meeting_session, meeting: current_meeting, scheduled_date: current_meeting.header_date)
      end

      def create_previous_meeting_session
        previous_begin_date = Date.new(2998, 1, 1)
        previous_end_date = Date.new(2998, 12, 31)
        previous_season = create(
          :season,
          season_type_id: 8,
          begin_date: previous_begin_date,
          end_date: previous_end_date,
          header_year: "#{previous_begin_date.year}/#{previous_end_date.year}"
        )
        previous_meeting = create(
          :meeting,
          season: previous_season,
          header_date: previous_begin_date + 120.days
        )
        create(:meeting_session, meeting: previous_meeting, scheduled_date: previous_meeting.header_date)
      end

      it 'exposes old timing and meeting fields in the view schema' do
        expect(described_class.column_names).to include(
          'old_meeting_individual_result_id', 'old_meeting_id', 'old_meeting_date', 'old_meeting_name',
          'old_total_hundredths', 'old_minutes', 'old_seconds', 'old_hundredths'
        )
      end

      it 'keeps old payload coherent when old result is present' do # rubocop:disable RSpec/ExampleLength
        current_session = create_current_meeting_session
        current_program = create(
          :meeting_program,
          meeting_session: current_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        current_mir = create(
          :meeting_individual_result,
          meeting_program: current_program,
          minutes: 0,
          seconds: 33,
          hundredths: 95,
          disqualified: false
        )

        previous_session = create_previous_meeting_session
        previous_program = create(
          :meeting_program,
          meeting_session: previous_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        old_mir = create(
          :meeting_individual_result,
          meeting_program: previous_program,
          swimmer: current_mir.swimmer,
          team: current_mir.team,
          team_affiliation: current_mir.team_affiliation,
          minutes: 0,
          seconds: 34,
          hundredths: 14,
          disqualified: false
        )

        row = described_class.where(
          swimmer_id: current_mir.swimmer_id,
          event_type_id: event_type.id,
          pool_type_id: pool_type.id
        ).first

        expect(row).not_to be_nil

        expect(row.old_meeting_id).not_to be_nil
        expect(row.old_meeting_date).not_to be_nil
        expect(row.old_meeting_name).not_to be_nil
        expect(row.old_total_hundredths).not_to be_nil

        expected_old_total = (row.old_minutes.to_i * 6000) + (row.old_seconds.to_i * 100) + row.old_hundredths.to_i
        expect(row.old_total_hundredths.to_i).to eq(expected_old_total)

        old_result = GogglesDb::MeetingIndividualResult.find(row.old_meeting_individual_result_id)
        old_meeting = old_result.meeting_program.meeting_event.meeting_session.meeting

        expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        expect(old_result.minutes).to eq(row.old_minutes)
        expect(old_result.seconds).to eq(row.old_seconds)
        expect(old_result.hundredths).to eq(row.old_hundredths)
        expect(old_meeting.id).to eq(row.old_meeting_id)
        expect(old_meeting.header_date).to eq(row.old_meeting_date)
        expect(old_meeting.description).to eq(row.old_meeting_name)
      end

      it 'keeps all old payload fields nil when baseline is missing' do # rubocop:disable RSpec/ExampleLength
        current_session = create_current_meeting_session
        current_program = create(
          :meeting_program,
          meeting_session: current_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        current_mir = create(
          :meeting_individual_result,
          meeting_program: current_program,
          minutes: 0,
          seconds: 35,
          hundredths: 20,
          disqualified: false
        )

        row = described_class.where(
          swimmer_id: current_mir.swimmer_id,
          event_type_id: event_type.id,
          pool_type_id: pool_type.id
        ).first

        expect(row).not_to be_nil

        expect(row.old_meeting_id).to be_nil
        expect(row.old_meeting_date).to be_nil
        expect(row.old_meeting_name).to be_nil
        expect(row.old_total_hundredths).to be_nil
        expect(row.old_minutes).to be_nil
        expect(row.old_seconds).to be_nil
        expect(row.old_hundredths).to be_nil
      end
    end

    describe 'championship year season selection' do
      let(:event_type) { GogglesDb::EventType.find(11) }
      let(:gender_type) { GogglesDb::GenderType.male }
      let(:pool_type) { GogglesDb::PoolType.find(1) }

      def create_season_with_meeting(begin_date, end_date, season_type_id = 1)
        season = create(
          :season,
          season_type_id: season_type_id,
          begin_date: begin_date,
          end_date: end_date,
          header_year: "#{begin_date.year}/#{end_date.year}"
        )
        meeting = create(
          :meeting,
          season: season,
          header_date: begin_date + 90.days
        )
        create(:meeting_session, meeting: meeting, scheduled_date: meeting.header_date)
      end

      def create_result_in_session(meeting_session, swimmer: nil, team: nil, team_affiliation: nil, minutes: 0, seconds: 33, hundredths: 0) # rubocop:disable Metrics/ParameterLists
        program = create(
          :meeting_program,
          meeting_session: meeting_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        attrs = {
          meeting_program: program,
          minutes: minutes,
          seconds: seconds,
          hundredths: hundredths,
          disqualified: false
        }
        attrs[:swimmer] = swimmer if swimmer
        attrs[:team] = team if team
        attrs[:team_affiliation] = team_affiliation if team_affiliation
        create(:meeting_individual_result, **attrs)
      end

      context 'with a summer-season current season (Jun-Aug start)' do
        it 'finds previous result from championship_year - 1' do # rubocop:disable RSpec/ExampleLength
          # Current: Jun 2999 → Aug 3000, championship_year = YEAR(end_date) = 3000
          current_session = create_season_with_meeting(
            Date.new(2999, 6, 1), Date.new(3000, 8, 31)
          )
          current_mir = create_result_in_session(current_session, seconds: 32)

          # Previous: Jun 2998 → Aug 2999, championship_year = YEAR(end_date) = 2999 = 3000 - 1
          previous_session = create_season_with_meeting(
            Date.new(2998, 6, 1), Date.new(2999, 8, 31), 8
          )
          old_mir = create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 34
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        end
      end

      context 'with a standard-season current season (Sep start)' do
        it 'finds previous result from championship_year - 1' do # rubocop:disable RSpec/ExampleLength
          # Current: Sep 2999 → Jun 3000, championship_year = YEAR(begin_date) = 2999
          current_session = create_season_with_meeting(
            Date.new(2999, 9, 1), Date.new(3000, 6, 30)
          )
          current_mir = create_result_in_session(current_session, seconds: 31)

          # Previous: Sep 2998 → Jun 2999, championship_year = YEAR(begin_date) = 2998 = 2999 - 1
          previous_session = create_season_with_meeting(
            Date.new(2998, 9, 1), Date.new(2999, 6, 30), 8
          )
          old_mir = create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 35
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        end
      end

      context 'with a spring-season current season (Jan-May start)' do
        it 'finds previous result from championship_year - 1' do # rubocop:disable RSpec/ExampleLength
          # Current: Feb 3001 → May 3001, championship_year = YEAR(begin_date) - 1 = 3000
          current_session = create_season_with_meeting(
            Date.new(3001, 2, 1), Date.new(3001, 5, 31)
          )
          current_mir = create_result_in_session(current_session, seconds: 30)

          # Previous: Jun 2998 → Aug 2999, championship_year = YEAR(end_date) = 2999 = 3000 - 1
          previous_session = create_season_with_meeting(
            Date.new(2998, 6, 1), Date.new(2999, 8, 31), 8
          )
          old_mir = create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 36
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        end
      end

      context 'with a summer season sharing the same championship year as a standard season' do
        it 'selects the faster MIR from the summer season as current best' do # rubocop:disable RSpec/ExampleLength
          # Standard season: Sep 2999 → Jun 3000, championship_year = YEAR(begin_date) = 2999
          standard_session = create_season_with_meeting(
            Date.new(2999, 9, 1), Date.new(3000, 6, 30)
          )
          standard_mir = create_result_in_session(standard_session, seconds: 35)

          # Summer season: Jun 2999 → Aug 2999, championship_year = YEAR(end_date) = 2999
          # Same championship year as the standard season above
          summer_session = create_season_with_meeting(
            Date.new(2999, 6, 1), Date.new(2999, 8, 31), 8
          )
          summer_mir = create_result_in_session(
            summer_session,
            swimmer: standard_mir.swimmer,
            team: standard_mir.team,
            team_affiliation: standard_mir.team_affiliation,
            seconds: 33
          )

          row = described_class.where(
            swimmer_id: standard_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.meeting_individual_result_id).to eq(summer_mir.id)
          expect(row.total_hundredths).to eq(3300)
          expect(row.total_hundredths).to be < 3500
        end
      end
    end

    describe '3-year base timing window (via goggles_cup_3y_base_timings)' do
      let(:event_type) { GogglesDb::EventType.find(11) }
      let(:gender_type) { GogglesDb::GenderType.male }
      let(:pool_type) { GogglesDb::PoolType.find(1) }

      def create_season_with_meeting(begin_date, end_date, season_type_id = 1)
        season = create(
          :season,
          season_type_id: season_type_id,
          begin_date: begin_date,
          end_date: end_date,
          header_year: "#{begin_date.year}/#{end_date.year}"
        )
        meeting = create(
          :meeting,
          season: season,
          header_date: begin_date + 90.days
        )
        create(:meeting_session, meeting: meeting, scheduled_date: meeting.header_date)
      end

      def create_result_in_session(meeting_session, swimmer: nil, team: nil, team_affiliation: nil, minutes: 0, seconds: 33, hundredths: 0) # rubocop:disable Metrics/ParameterLists
        program = create(
          :meeting_program,
          meeting_session: meeting_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        attrs = {
          meeting_program: program,
          minutes: minutes,
          seconds: seconds,
          hundredths: hundredths,
          disqualified: false
        }
        attrs[:swimmer] = swimmer if swimmer
        attrs[:team] = team if team
        attrs[:team_affiliation] = team_affiliation if team_affiliation
        create(:meeting_individual_result, **attrs)
      end

      # Current season: Sep 3003 → Jun 3004, championship_year = 3003
      # 3-year window: championship_year BETWEEN 3000 AND 3002

      context 'when a result exists from championship_year - 2' do
        it 'finds the base timing from 2 years back' do # rubocop:disable RSpec/ExampleLength
          current_session = create_season_with_meeting(
            Date.new(3003, 9, 1), Date.new(3004, 6, 30)
          )
          current_mir = create_result_in_session(current_session, seconds: 30)

          # Sep 3001 → Jun 3002, championship_year = 3001 = 3003 - 2 (within window)
          previous_session = create_season_with_meeting(
            Date.new(3001, 9, 1), Date.new(3002, 6, 30), 8
          )
          old_mir = create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 34
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        end
      end

      context 'when a result exists from championship_year - 3' do
        it 'finds the base timing from 3 years back (outer boundary)' do # rubocop:disable RSpec/ExampleLength
          current_session = create_season_with_meeting(
            Date.new(3003, 9, 1), Date.new(3004, 6, 30)
          )
          current_mir = create_result_in_session(current_session, seconds: 30)

          # Sep 3000 → Jun 3001, championship_year = 3000 = 3003 - 3 (outer boundary)
          previous_session = create_season_with_meeting(
            Date.new(3000, 9, 1), Date.new(3001, 6, 30), 8
          )
          old_mir = create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 36
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
        end
      end

      context 'when a result exists only from championship_year - 4' do
        it 'does not find a base timing (outside 3-year window)' do # rubocop:disable RSpec/ExampleLength
          current_session = create_season_with_meeting(
            Date.new(3003, 9, 1), Date.new(3004, 6, 30)
          )
          current_mir = create_result_in_session(current_session, seconds: 30)

          # Sep 2999 → Jun 3000, championship_year = 2999 = 3003 - 4 (outside window)
          previous_session = create_season_with_meeting(
            Date.new(2999, 9, 1), Date.new(3000, 6, 30), 8
          )
          create_result_in_session(
            previous_session,
            swimmer: current_mir.swimmer,
            team: current_mir.team,
            team_affiliation: current_mir.team_affiliation,
            seconds: 38
          )

          row = described_class.where(
            swimmer_id: current_mir.swimmer_id,
            event_type_id: event_type.id,
            pool_type_id: pool_type.id
          ).first

          expect(row).not_to be_nil
          expect(row.old_meeting_individual_result_id).to be_nil
          expect(row.old_total_hundredths).to be_nil
        end
      end
    end

    describe '.with_base_year' do
      let(:event_type) { GogglesDb::EventType.find(11) }
      let(:gender_type) { GogglesDb::GenderType.male }
      let(:pool_type) { GogglesDb::PoolType.find(1) }

      def create_season_with_meeting(begin_date, end_date, season_type_id = 1)
        season = create(
          :season,
          season_type_id: season_type_id,
          begin_date: begin_date,
          end_date: end_date,
          header_year: "#{begin_date.year}/#{end_date.year}"
        )
        meeting = create(
          :meeting,
          season: season,
          header_date: begin_date + 90.days
        )
        create(:meeting_session, meeting: meeting, scheduled_date: meeting.header_date)
      end

      def create_result_in_session(meeting_session, swimmer: nil, team: nil, team_affiliation: nil, seconds: 33)
        program = create(
          :meeting_program,
          meeting_session: meeting_session,
          event_type: event_type,
          gender_type: gender_type,
          pool_type: pool_type
        )
        attrs = {
          meeting_program: program,
          minutes: 0,
          seconds: seconds,
          hundredths: 0,
          disqualified: false
        }
        attrs[:swimmer] = swimmer if swimmer
        attrs[:team] = team if team
        attrs[:team_affiliation] = team_affiliation if team_affiliation
        create(:meeting_individual_result, **attrs)
      end

      after { described_class.with_base_year.to_a }

      it 'uses the provided base year to select the current championship season' do # rubocop:disable RSpec/ExampleLength
        current_session = create_season_with_meeting(
          Date.new(3000, 9, 1), Date.new(3001, 6, 30)
        )
        current_mir = create_result_in_session(current_session, seconds: 32)
        previous_session = create_season_with_meeting(
          Date.new(2999, 9, 1), Date.new(3000, 6, 30), 8
        )
        old_mir = create_result_in_session(
          previous_session,
          swimmer: current_mir.swimmer,
          team: current_mir.team,
          team_affiliation: current_mir.team_affiliation,
          seconds: 34
        )

        row = described_class.with_base_year(3000).where(
          swimmer_id: current_mir.swimmer_id,
          event_type_id: event_type.id,
          pool_type_id: pool_type.id
        ).first

        expect(row).not_to be_nil
        expect(row.meeting_individual_result_id).to eq(current_mir.id)
        expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
      end

      it 'shifts the 3-year window when a different base year is provided' do # rubocop:disable RSpec/ExampleLength
        current_session = create_season_with_meeting(
          Date.new(2999, 9, 1), Date.new(3000, 6, 30), 8
        )
        current_mir = create_result_in_session(current_session, seconds: 32)
        old_session = create_season_with_meeting(
          Date.new(2998, 9, 1), Date.new(2999, 6, 30), 8
        )
        old_mir = create_result_in_session(
          old_session,
          swimmer: current_mir.swimmer,
          team: current_mir.team,
          team_affiliation: current_mir.team_affiliation,
          seconds: 34
        )

        row = described_class.with_base_year(2999).where(
          swimmer_id: current_mir.swimmer_id,
          event_type_id: event_type.id,
          pool_type_id: pool_type.id
        ).first

        expect(row).not_to be_nil
        expect(row.meeting_individual_result_id).to eq(current_mir.id)
        expect(row.old_meeting_individual_result_id).to eq(old_mir.id)
      end

      it 'returns no row when the base year has no matching current season' do
        current_session = create_season_with_meeting(
          Date.new(3000, 9, 1), Date.new(3001, 6, 30)
        )
        current_mir = create_result_in_session(current_session, seconds: 32)

        rows = described_class.with_base_year(2998).where(
          swimmer_id: current_mir.swimmer_id,
          event_type_id: event_type.id,
          pool_type_id: pool_type.id
        )

        expect(rows).to be_empty
      end
    end
  end
end
