...
module GogglesDb
  RSpec.describe EventsByPoolType, type: :model do
    ...
    context 'any pre-seeded instance' do
      ...
      it 'is valid' do
        expect(subject).to be_an(EventsByPoolType).and be_valid
      end
    ...
    describe 'self.by_pool' do
      it_behaves_like('sorting scope by_<ANY_ENTITY_NAME>', EventsByPoolType, 'pool_type', 'length_in_meters')
    end

    describe 'self.by_event' do
      it_behaves_like('sorting scope by_<ANY_ENTITY_NAME>', EventsByPoolType, 'event_type', 'style_order')
    end
