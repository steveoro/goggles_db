...
module GogglesDb
  RSpec.describe DbFinders::FuzzyCity, type: :strategy do
    let(:fixture_row) { GogglesDb::City.first(100).sample }

    describe 'any instance' do
      subject { described_class.new(name: fixture_row.name) }
    ...
    context 'when using valid parameters' do
      ...
      describe '#scan_for_matches finding multiple results (1:N),' do
        [
          # 1:N matches:
          { area: 'reggio emilia', country_code: 'IT' },
          { area: 'modena', country_code: 'IT' }
        ].each do |fixture_row|
          describe "#call ('#{fixture_row.inspect}')" do
            subject { described_class.new(fixture_row) }
