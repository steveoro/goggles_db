...
module GogglesDb
  RSpec.describe CmdFindIsoCountry, type: :command do
    context 'when using valid parameters,' do
      [
        ...
        describe "#call('#{fixture_name}', '#{fixture_alpha2}')" do
          ...
          subject { CmdFindIsoCountry.call(fixture_name, fixture_alpha2) }
          let(:result_country) { subject.result }

          it 'returns itself' do
            expect(subject).to be_a(CmdFindIsoCountry)
          end
          it 'is successful' do
    ...
    context 'when using invalid parameters,' do
      ...
      describe '#call' do
        subject { CmdFindIsoCountry.call(fixture_name, fixture_code) }
