...
-- CTE to get the base year used for the championship window.
...
-- CTE to get all seasons with championship_year between base - 3 and base - 1
Previous3ySeasons AS (
  SELECT swc.id
  FROM SeasonWithChampionship swc
  JOIN BaseYear byr ON swc.championship_year BETWEEN byr.base_year - 3 AND byr.base_year - 1
),
...
-- Rank results within each swimmer/event/pool tuple across the 3-year window
RankedResults AS (
  SELECT
    vr.*,
    ROW_NUMBER() OVER (
  ...
  JOIN Previous3ySeasons ps ON ps.id = vr.season_id
)
-- Final selection of the best results (one row per swimmer/event/pool)
SELECT
  swimmer_id,
  swimmer_name,
