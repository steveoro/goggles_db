...
-- CTE to get previous season by chronology (across any season type)
PreviousSeason AS (
  SELECT s.id
  FROM seasons s
  JOIN CurrentSeason cs ON s.begin_date < cs.begin_date
  ORDER BY s.begin_date DESC, s.id DESC
  LIMIT 1
),
...
-- Previous season best result by swimmer/event/pool
PreviousSeasonRanked AS (
  ...
  FROM ValidResults vr
  JOIN PreviousSeason ps ON ps.id = vr.season_id
),
CurrentBest AS (
  SELECT *
