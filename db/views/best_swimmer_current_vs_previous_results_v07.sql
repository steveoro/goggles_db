...
-- CTE to get the base year used for the current championship selection.
-- The base year can be overridden by the consuming application by setting the
-- @base_year session variable (e.g. SET @base_year = 2025;) before querying the view.
-- When no override is set, the latest ongoing championship year is used.
BaseYear AS (
  SELECT goggles_db_base_year() AS base_year
),
-- CTE to get all seasons sharing the base championship year
CurrentSeasons AS (
  SELECT swc.id, swc.championship_year
  FROM SeasonWithChampionship swc
  JOIN BaseYear byr ON swc.championship_year = byr.base_year
),
...
-- Base championship year best result by swimmer/event/pool
...
SELECT
  ...
  cb.team_id,
  cb.team_name,
  bt.old_meeting_individual_result_id,
  bt.old_meeting_id,
  bt.old_meeting_date,
  bt.old_meeting_name,
  bt.old_total_hundredths,
  bt.old_minutes,
  bt.old_seconds,
  bt.old_hundredths
FROM CurrentBest cb
LEFT JOIN (
  ...
  FROM goggles_cup_3y_base_timings
) bt
  ON bt.swimmer_id = cb.swimmer_id
  AND bt.event_type_id = cb.event_type_id
  AND bt.pool_type_id = cb.pool_type_id;
